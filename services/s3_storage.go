...
func (s *S3Storage) GetTouch(ctx context.Context, hash string) (*s3.GetObjectOutput, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrapf(err, "failed to get check touch bucket=%v key=%v", s.bucket, key)
	}
	defer o.Body.Close()
	return o, nil
}
...
func (s *S3Storage) IsDone(ctx context.Context, hash string) (bool, error) {
	...
	if err != nil {
		...
		return false, errors.Wrapf(err, "failed to get done status bucket=%v key=%v", s.bucket, key)
	}
	defer o.Body.Close()
	return true, nil
}
...
func (s *S3Storage) IsTranscoded(ctx context.Context, hash string) (bool, error) {
	...
	if err != nil {
		...
		return false, errors.Wrapf(err, "failed to get transcoded status bucket=%v key=%v", s.bucket, key)
	}
	defer o.Body.Close()
	return true, nil
}
